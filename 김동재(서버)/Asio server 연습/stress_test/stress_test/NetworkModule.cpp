...
//#include "..\..\ASIO_GAME_SERVER\ASIO_GAME_SERVER\protocol.h"
...
void ProcessPacket(int ci, unsigned char packet[])
{
	switch (packet[1]) {
	case SC_POS: {
		sc_packet_pos* move_packet = reinterpret_cast<sc_packet_pos*>(packet);
	...
	case SC_LOGIN_INFO:
	{
		...
		active_clients++;
		sc_packet_login_info* login_packet = reinterpret_cast<sc_packet_login_info*>(packet);
		int my_id = ci;
