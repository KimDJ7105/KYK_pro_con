...
void ProcessPacket(char* ptr)
{
	...
	case SC_LOGIN_INFO:
	{
		sc_packet_login_info * packet = reinterpret_cast<sc_packet_login_info*>(ptr);
		g_myid = packet->id;
		avatar.id = g_myid;
	...
	case SC_TARGET_HIT :
	{
		...
		if (my_packet->shoter_id == g_myid) {
			if (my_packet->target_id < MAX_USER) {
				std::cout << "당신이 Player" << my_packet->target_id << "를 쐈습니다.\n";
			}

			else {
				std::cout << "당신이 NPC" << my_packet->target_id << "를 쐈습니다.\n";
			}
		}

		else {
			std::cout << "Player" << my_packet->shoter_id << "가 당신을 쐈습니다.\n";
		}
		break;
	}
	case SC_GAME_OVER : 
...
int main()
{
	...
	sf::Socket::Status status = s_socket.connect("127.0.0.1", MY_SERVER_PORT);
